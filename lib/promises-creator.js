...
function createTempFolderSync(tempPath = os.tmpdir()) {
	return fs.mkdtempSync(path.join(tempPath, 'atom_python_linters'));
}

async function createTempFolder(tempPath = os.tmpdir()) {
	return new Promise((resolve, reject) => {
...
module.exports = {fileReadableAsync, removeFromDisk, removeFromDiskContent, createTempFolder, createTempFolderSync, createTextFile};
